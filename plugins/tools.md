...
- [safety-net](https://github.com/JohnXu22786/safety-net) — 破坏性命令拦截闸门：rm -rf / git reset --hard / push --force 执行前解析判定并要求人工确认（dsh 插件 + 通用 CLI）
- [command-scout](https://github.com/JohnXu22786/command-scout) — 扫描项目声明的构建命令（Makefile / package.json scripts / justfile / deno tasks）并暴露为 agent 工具
